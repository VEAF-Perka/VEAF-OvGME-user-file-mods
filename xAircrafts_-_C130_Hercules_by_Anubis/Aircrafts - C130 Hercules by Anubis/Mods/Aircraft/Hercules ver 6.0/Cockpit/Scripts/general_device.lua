...
dofile(LockOn_Options.script_path.."mission.lua")
...
------------------------------------------------------------------------ Clock
Clock_hours = get_param_handle("Clock_hours")
Clock_mins = get_param_handle("Clock_mins")
Clock_secs = get_param_handle("Clock_secs")
...
local current_Gforce = get_param_handle("CURRENT_Gforce")
------------------------------------------------------------------------ force_feedback
-- local force_feedback_enabled = get_param_handle("force_feedback_enabled")
-- if get_option_value("miscellaneous.force_feedback_enabled","local") then
	-- force_feedback_enabled:set(1)
-- end
...
-- local Kneeboard_Page_Forward = get_param_handle("Kneeboard_Page_Forward")
-- local Kneeboard_Page_Back = get_param_handle("Kneeboard_Page_Back")
-- local Kneeboard_Mark = get_param_handle("Kneeboard_Mark")
-- local Kneeboard_Auto_Mark = false

----------------------------- Birth_Lat, Birth_Long, Birth_Year, Birth_Month, Birth_Day, Birth_Time (for sunset/sunrise calculations)
...
------------------------------------------------------------------------ Clock
    ...
    Clock_hours:set(hours12)
    local int,frac = math.modf(hours12)
    Clock_mins:set(frac * 60.0)
    int,frac_s = math.modf(frac * 60.0)
    Clock_secs:set(math.floor(frac_s * 60))
    -- print_message_to_user("hr:"..tostring(hours12)..",min:"..tostring(frac*60)..",sec:"..tostring(frac_s*60))
------------------------------------------------------------------------ 

	Heading:set(sensor_data.getHeading())
	current_Gforce:set(sensor_data.getVerticalAcceleration())
	...
	-- if (Kneeboard_Page_Forward:get() == 1) then
		-- GetDevice(devices.Kneeboard):performClickableAction(MISCELLANEOUS.Kneeboard_Page_Forward, 1)--Kneeboard_Page_Forward
		-- Kneeboard_Page_Forward:set(0)
		-- -- dispatch_action(nil,3000,0)--send 3000 to efm
	-- else
		-- if (Kneeboard_Page_Back:get() == 1) then
			-- GetDevice(devices.Kneeboard):performClickableAction(MISCELLANEOUS.Kneeboard_Page_Back, 1)--Kneeboard_Page_Back
			-- Kneeboard_Page_Back:set(0)
		-- else
			-- if (Kneeboard_Auto_Mark == false) then
				-- self_loc_x_old = self_loc_x
				-- self_loc_z_old = self_loc_z
				-- GetDevice(devices.Kneeboard):performClickableAction(MISCELLANEOUS.Kneeboard_Mark, 1)--Kneeboard_Mark
				-- GetDevice(devices.Kneeboard):performClickableAction(MISCELLANEOUS.Kneeboard_Mark, 0)--Kneeboard_Mark
				-- Kneeboard_Auto_Mark = true
			-- else
				-- if ((self_loc_x_old - 1000 > self_loc_x)
				-- or (self_loc_x_old + 1000 < self_loc_x)
				-- or (self_loc_z_old + 1000 < self_loc_z)
				-- or (self_loc_z_old - 1000 > self_loc_z)) then
					-- Kneeboard_Auto_Mark = false
				-- end
			-- end
			-- -- if (Kneeboard_Mark:get() == 1) then
				-- -- GetDevice(devices.Kneeboard):performClickableAction(MISCELLANEOUS.Kneeboard_Mark, 1)--Kneeboard_Mark
				-- -- Kneeboard_Mark:set(0)
				-- -- set_aircraft_draw_argument_value(627, 0)--Kneeboard_Mark button
			-- -- end
		-- end
	-- end

	-- if (Kneeboard_Auto_Mark <= 200) then
		-- Kneeboard_Auto_Mark = Kneeboard_Auto_Mark + 1
	-- else
		-- GetDevice(devices.Kneeboard):performClickableAction(MISCELLANEOUS.Kneeboard_Mark, 1)--Kneeboard_Mark
		-- print_message_to_user(string.format("Kneeboard_Auto_Mark  %d", Kneeboard_Auto_Mark))
		-- Kneeboard_Auto_Mark = 0
	-- end
	
		-- print_message_to_user(string.format("draw_argument_value(114)  %d", get_aircraft_draw_argument_value(114)))
	-- if (get_aircraft_draw_argument_value(114) > 0) then
		-- print_message_to_user(string.format("get_aircraft_draw"))
	-- end
	-- if (get_cockpit_draw_argument_value(114) > 0) then
		-- print_message_to_user(string.format("get_cockpit_draw_"))
	-- end

	-- if (get_cockpit_draw_argument_value(802) > 0.6) then
		-- print_message_to_user(string.format("draw_argument_value(802)  %d", get_cockpit_draw_argument_value(802)))
		-- set_aircraft_draw_argument_value(38,get_cockpit_draw_argument_value(802))
	-- else
		-- set_aircraft_draw_argument_value(38,get_cockpit_draw_argument_value(802))
	-- end
	----------------------------------------------------------------- LANDING_GEAR_LIGHTS_PANEL
	-- if (Gear_Lever_Pos:get() == 1) then
		-- GetDevice(devices.FLIGHTCONTROLS):performClickableAction(3461, 1)--fcs_commands.Handle_GEAR_LEVER
	-- else
		-- GetDevice(devices.FLIGHTCONTROLS):performClickableAction(3461, 0)
	-- end
	
end

function post_initialize()
-- Birth_Lat, Birth_Long, Birth_Year, Birth_Month, Birth_Day, Birth_Time, Birth_Map (for sunset/sunrise calculations)
